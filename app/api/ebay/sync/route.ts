...
export async function POST() {
  ...
  if (albumsError) {
    return NextResponse.json(
      { error: `Failed to load albums for sync: ${albumsError.message}` },
      { status: 500 }
    );
  }
  ...
  if (ebayCredsError) {
    return NextResponse.json(
      { error: `Failed to load eBay credentials: ${ebayCredsError.message}` },
      { status: 500 }
    );
  }
  ...
  if (isRealEbay && ebayCreds) {
    try {
      ...
      if (tokenResult.refreshed) {
        ...
        if (tokenUpdateError) {
          return NextResponse.json(
            { error: `Failed to save refreshed eBay token: ${tokenUpdateError.message}` },
            { status: 500 }
          );
        }
      }
  ...
  for (const row of albums ?? []) {
    ...
    if (outcome.changed) {
      ...
      if (updateError) {
        return NextResponse.json(
          {
            error: `Failed to save marketplace sync result for album ${album.id}: ${updateError.message}`,
          },
          { status: 500 }
        );
      }
